<?php
  ...
  date_default_timezone_set("America/Mexico_City");
    ...
    $hour = date("H:i:s");
		...
		$r = $mysqli->query("SELECT * FROM alumnos");
		    if ($r)
		      {
        	...
        	if ($correo_verificar == 1) {
        	  	...
        	  	$rr = $mysqli->query("SELECT * FROM alumnos WHERE correo = '$correo'");
		    	if ($rr)
		      	{
                ...
                $semestre = $arrayy->semestre;
        	  	...
        	  	if ($pw_verificar == 1) {
        	  		echo "Accediendo...";        	  		
        	  		$_SESSION['nombre_alumno'] = $nombre_bd;
                $_SESSION['correo_alumno'] = $correo;
                $_SESSION['semestre_alumno'] = $semestre;

	              echo '<script type="text/javascript">';
                echo 'window.location="../../secciones/estudiantes.php"';
                echo '</script>';
                echo '</body>';
